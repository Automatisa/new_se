...
insert  into `x_settings`(`so_name_vc`,`so_cleanname_vc`,`so_value_tx`,`so_defvalues_tx`,`so_desc_tx`,`so_module_vc`,`so_usereditable_en`) values ('bulwark_port','Bulwark Apache Port','80',NULL,'Bulwark Apache panel port','Bulwark Config','true');
...
ALTER TABLE `bulwark_roundcube`.`users` CHANGE `preferences` `preferences` longtext;
...
USE `bulwark_proftpd`; /* We need to make some changes to the ProFTPD DB schema also..*/
