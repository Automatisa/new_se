						...
						if (filter_var($detectedip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)){
							...
							if ($autoipsettings['ai_script_vc'] != ""){
								...
								if (sys_versions::ShowOSPlatformVersion() == "Windows"){
									...
									// Security fix (June 2026): the legacy code called
									// `zsudo <admin-supplied-path>` to run a panel-config
									// script as root. That is the same class of vulnerability
									// as the rest of the zsudo removal: an admin-set path
									// flowing into a shell command invoked by a privilege
									// boundary. The fix is to (a) whitelist the path to an
									// absolute path under /usr/local/bin/ and (b) drop
									// root entirely — IP-change notification scripts do not
									// need to run as root, so exec() under the web server
									// user is sufficient. If an admin truly needs root for
									// a custom script, they should configure a sudoers rule
									// for that specific absolute path themselves.
									$autoip_script = (string)$autoipsettings['ai_script_vc'];
									if (preg_match('#^/[A-Za-z0-9._/\-]+$#', $autoip_script)
									    && strpos($autoip_script, '/usr/local/bin/') === 0) {
										@exec($autoip_script);
									} else {
										echo "WARNING: autoip script path rejected (must be under /usr/local/bin/): " . $autoip_script . fs_filehandler::NewLine();
									}
								}
							}
						}
