...
function SuspendMailboxes() {
    ...
    // Directamente suspendidos + sub-clientes activos de resellers suspendidos
    $sql = $zdbh->prepare("
        SELECT mb.mb_address_vc
        FROM x_mailboxes mb
        JOIN x_accounts ac ON mb.mb_acc_fk = ac.ac_id_pk
        WHERE ac.ac_enabled_in = 0 AND ac.ac_suspended_in = 1 AND mb.mb_deleted_ts IS NULL
        UNION
        SELECT mb.mb_address_vc
        FROM x_mailboxes mb
        JOIN x_accounts ac ON mb.mb_acc_fk = ac.ac_id_pk
        JOIN x_accounts res ON ac.ac_reseller_fk = res.ac_id_pk
        WHERE res.ac_enabled_in = 0 AND res.ac_suspended_in = 1
